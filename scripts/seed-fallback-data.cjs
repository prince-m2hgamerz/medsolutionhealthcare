...
async function main() {
  ...
  // 1. Load fallback data from lib/doctors-data.ts (compiled via tsc)
  console.log("Loading fallback data...");
  const fallback = require(path.join(__dirname, "..", "tmp-data", "doctors-data"));
  const allDoctors = fallback.allDoctors;
  const hospitalImageBySlug = fallback.hospitalImageBySlug;
  ...
  const hospitalSlugToId = new Map();
  ...
  // 4. Deduplicate by slug (merge specialties for same doctor at same hospital)
  console.log("Deduplicating doctors by slug...");
  const deduped = new Map();
  for (const d of allDoctors) {
    const existing = deduped.get(d.slug);
    if (existing) {
      existing.specialties.push(d.specialty);
      if (d.telephone && !existing.telephone) existing.telephone = d.telephone;
      if (d.profileUrl && !existing.profileUrl) existing.profileUrl = d.profileUrl;
      if (d.appointmentUrl && !existing.appointmentUrl) existing.appointmentUrl = d.appointmentUrl;
    } else {
      deduped.set(d.slug, { ...d, specialties: [d.specialty] });
    }
  }
  const unique = [...deduped.values()];
  console.log(`  ${allDoctors.length} -> ${unique.length} unique doctors\n`);

  // 5. Upsert all doctors in batches
  ...
  for (let i = 0; i < unique.length; i += BATCH_SIZE) {
    ...
    if ((Math.floor(i / BATCH_SIZE) + 1) % 5 === 0 || i + BATCH_SIZE >= unique.length) {
      console.log(`  Processed ${Math.min(i + BATCH_SIZE, unique.length)}/${unique.length} doctors (${inserted} upserted, ${skipped} skipped, ${errors} batch errors)`);
    }
  }
